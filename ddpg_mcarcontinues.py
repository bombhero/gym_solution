...
class ActorCriticAgent:
    ...
    def training(self, observations, rewards, targets=None):
        ...
        actions_tensor = self.actor_net(observe_tensor)
...
def main():
    ...
    for i in range(100):
        observe_batch = np.random.random([256, 4]) * 2 - 1
        target_batch = np.zeros([256, 1])

        for i in range(target_batch.shape[0]):
            target_batch[i, 0] = sum(observe_batch[i]) / 4
